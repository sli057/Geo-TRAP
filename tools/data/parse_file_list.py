...
def parse_directory(path, # 'data/ucf101/rawframes/'
                    rgb_prefix='img_', # 'frame'
    ...
    if level == 1:
        ...
        frame_dirs = glob.glob(osp.join(path, '*')) # 'data/ucf101/rawframes/v_YoYo_g25_c05'
    ...
    for i, frame_dir in enumerate(frame_dirs):
        ...
        #print(total_num)
        dir_name = locate_directory(frame_dir) # v_YoYo_g25_c05
        ...
        # {v_YoYo_g25_c05:('data/ucf101/rawframes/v_YoYo_g25_c05', 108, 0)
...
def parse_ucf101_splits(level):
    ...
    def line_to_map(line):
        ...
        video = osp.splitext(items[0])[0] # 'YoYo/v_YoYo_g21_c02'
        if level == 1:
            video = osp.basename(video) # v_YoYo_g21_c02
            label = class_mapping[osp.dirname(items[0])]#items[0]  changed by Shasha
        elif level == 2:
            video = osp.join(
                osp.basename(osp.dirname(video)), osp.basename(video))
            label = class_mapping[osp.dirname(items[0])]
        return video, label # v_YoYo_g21_c02, 100
