...
class ResNet3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.inplanes = self.base_channels # 64
        ...
        for i, num_blocks in enumerate(self.stage_blocks):  # (3, 4, 6, 3)
            ...
            planes = self.base_channels * 2**i  # (64, 64*2, 64*4, 64*8)
            res_layer = self.make_res_layer(
                self.block,  # Bottleneck3d
                self.inplanes, # [64, 64*4, 64*8, 64*16]
                planes,        # (64, 64*2, 64*4, 64*8) [inplanes--> plannes --> planes//expansion]
                num_blocks,  # (3, 4, 6, 3)
                ...
                **kwargs)
            self.inplanes = planes * self.block.expansion  # expansion=4
            layer_name = f'layer{i + 1}'
            self.add_module(layer_name, res_layer)
            self.res_layers.append(layer_name)

        self.feat_dim = self.block.expansion * self.base_channels * 2**(
            len(self.stage_blocks) - 1) #4*64*8
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
            if i == 0 and self.with_pool2:  # True
                x = self.pool2(x)
...
if __name__ == '__main__':
    res = ResNet3d(depth=50,
                        ...
                        conv1_stride_t=1,
                        pool1_stride_t=1,
                        inflate=(0, 0, 1, 1)).cuda()  # reversed
    print('model built')
    #res_dec.to('cuda')
    summary(res, input_size=(3, 2, 112, 112))
    # x = torch.tensor(np.random.rand(1, 64 * 16, 2, 7, 7), dtype=torch.float).cuda()
    # p = res_dec(x)
    # print(np.shape(p))
    # print(p.size())
