...
def load_checkpoint(model,
    ...
    Args:
        ...
        prefix: prefix to be stripped for state_dict
...
class ResNet3dPathway(ResNet3d):
    ...
    def __init__(self,
        ...
        self.inplanes = self.base_channels # 64 / 8
        if self.lateral:
            self.conv1_lateral = ConvModule(
                ...
                norm_cfg=None,
                act_cfg=None)
            # torch.nn.Conv3d(in_channels=8, out_channels=16,
            #     kernel_size=[5,1,1],
            #     stride=[8,1,1],
            #     padding=[2,1,1],
            #     dilation=1,
            ...
            # no norm
            # no activation
            # [N,8,T,H,W] --> [N,16,T/8,H,W]

        self.lateral_connections = []
        for i in range(len(self.stage_blocks)):  # stage_block=(3, 4, 6, 3)
            planes = self.base_channels * 2**i   # [64, 64*2, 64*4, 64*8]
            self.inplanes = planes * self.block.expansion  # self.block=BasicBlock3d() .expansion=4
...
class ResNet3dSlowFast(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.slow_path.res_layers):
            ...
            if not self.dec_use or i != len(self.slow_path.res_layers)-1:
            ...
            if (i != len(self.slow_path.res_layers) - 1
                ...
                if not self.dec_use or i != len(self.slow_path.res_layers)-2:
                    x_slow = torch.cat((x_slow, x_fast_lateral), dim=1)
