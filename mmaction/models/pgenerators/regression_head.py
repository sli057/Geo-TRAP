...
class RegressionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_action_fc = use_action_fc
        ...
        if use_resize is not None:
            print('Note: use_resize is deprecated. Use num_freedom instead')
        # if use_resize:
        #     assert num_freedom == 4
        # else:
        #     assert num_freedom == 3

        if self.use_two_layers:
            self.fc_cls1 = nn.Linear(in_channels, in_channels//8, bias=False) # 32*8 --> 32 --> 6
            self.bn_cls1 = nn.BatchNorm1d(in_channels//8)
            self.fc_cls2 = nn.Linear(in_channels//8, num_freedom, bias=False)
            self.bn_cls2 = nn.BatchNorm1d(num_freedom)
        else:
            self.fc_cls = nn.Linear(in_channels, num_freedom)
    ...
    def init_weights(self):
        ...
        if self.use_two_layers:
            ...
            identity_init(self.fc_cls2)
        else:
            identity_init(self.fc_cls)
    ...
    def forward(self, x):
        ...
        if self.use_two_layers:
            ...
            x = self.fc_cls(x)
        #print(x[0])
        if self.use_action_fc:
            x = torch.tanh(x)
        # print('\n', x[:, 0, -2])
        # print(x[:, 0, -2])
        # [N, 16, num_freedom]
        if self.use_resize is False:
            x = nn.functional.pad(x, (0, 1))
        return x


class RegressionHeadNoneCu(nn.Module):
    """The classification head for SlowFast.
    ...
    def __init__(self,
        ...
        if self.use_two_layers:
            self.fc_cls1 = nn.Linear(16*in_channels, 16*in_channels//8, bias=False) # 32*8 --> 32 --> 6
            self.bn_cls1 = nn.BatchNorm1d(in_channels//8)
            self.fc_cls2 = nn.Linear(in_channels//8, num_freedom, bias=False)
            self.bn_cls2 = nn.BatchNorm1d(num_freedom)
        else:
...
class AffineRegressionHead(nn.Module):
    """The classification head for SlowFast.
    ...
    def __init__(self,
        ...
        self.fc_cls1 = nn.Linear(in_channels, in_channels//8, bias=True) # 32*8 --> 32 --> 6
        # self.bn_cls1 = nn.BatchNorm1d(in_channels//8)
...
class AffineRegressionHeadNoneCu(nn.Module):
    """The classification head for SlowFast.
    ...
    def forward(self, x):
        ...
        x = torch.relu(x)
        x = x.view(N, T, -1)
        x = self.fc_cls2(x) #[N, 16*6]
        # x = x.transpose(-1, -2)
        # x = self.bn_cls2(x)
        # x = x.transpose(-1, -2)
        return x

class AffineRegressionHeadNoneCuAdjust(nn.Module):
    """The classification head for SlowFast.
