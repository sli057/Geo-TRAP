...
from mmaction.core.runner.pair_runner import grouper, compose_pair_batch
import numpy as np

def single_gpu_test_MP(model, data_loader, max_test=None):
    ...
    if max_test:
        ...
        prog_bar = mmcv.ProgressBar(len(dataset)//2)
    for ind, data_batch_pair in enumerate(grouper(data_loader, n=2)):
        if max_test and ind >= max_test:
            break
        ...
        with torch.no_grad():
            kwargs = {}
            result = model(data_batch, return_loss=False, **kwargs)
            #print(result)
            #print(np.shape(result)) # [15, 101]
