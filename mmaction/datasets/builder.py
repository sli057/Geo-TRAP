...
class SamplerMP(Sampler):
    ...
    def __iter__(self):
        assert self.data_source.video_infos_by_class is not None
        indice = list(self.data_source.video_infos_by_class.keys())
        np.random.shuffle(indice)
        indice = np.tile(indice, (self.batch_size, 1))
        indice = np.reshape(np.transpose(indice), [-1])
        return iter(indice)
...
def build_dataset(cfg, default_args=None):
    ...
    """
    if cfg['type'] == 'RepeatDataset': #'RawframeDataset'
...
def build_dataloader(dataset,
    ...
    if dist:
        ...
        if sample_by_class:
            sampler = SamplerMP(dataset, batch_size)
            shuffle = None
        else:
            sampler = None
