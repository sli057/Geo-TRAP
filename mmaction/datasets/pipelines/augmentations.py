...
class NormalizeRGBFlow:
    ...
    def __call__(self, results):
        ...
        if modality == 'Flow':
            num_imgs = len(results['imgs'])
            assert num_imgs % 2 == 0
            mean = np.array([128, 128], dtype=np.float32) # added by shasha
            std = np.array([128, 128], dtype=np.float32) # added by shasha
            ...
            for i in range(n):
                x_flow[i] = results['imgs'][2 * i]
                y_flow[i] = results['imgs'][2 * i + 1]
            x_flow = (x_flow - mean[0]) / std[0]
            y_flow = (y_flow - mean[1]) / std[1]
            if self.adjust_magnitude:
                x_flow = x_flow * results['scale_factor'][0]
                y_flow = y_flow * results['scale_factor'][1]
            imgs = np.stack([x_flow, y_flow], axis=-1)
