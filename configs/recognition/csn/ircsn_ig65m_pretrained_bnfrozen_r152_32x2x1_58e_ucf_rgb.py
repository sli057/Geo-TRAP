...
val_pipeline = [
    dict(
        type='SampleFrames',
        clip_len=16,  # changed32,
        frame_interval=1,  # changed 2,
        num_clips=1,
        test_mode=True),
    dict(type='FrameSelector'),
    dict(type='Resize', scale=(128, -1)),  # (-1, 256)), # we need 128
    dict(type='CenterCrop', crop_size=112),  # type='ThreeCrop', crop_size=256),
...
# optimizer
optimizer = dict(
    type='SGD', lr=0.000125, momentum=0.9, # learning rate okay??
...
log_level = 'INFO'
work_dir = f'./work_dirs/ircsn_ig65m_pretrained_bnfrozen_r152_32x2x1_58e_ucf101_{split}_rgb'  # noqa: E501
load_from = None
resume_from = None
